...
@implementation testandroidmapObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_testandroidmap);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any testandroidmap patches in JS before a context is loaded... nice for 
...
{
	// create testandroidmap global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_testandroidmap = [[testandroidmapObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_testandroidmap];
	
	NSString *_testandroidmapNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _testandroidmapNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_testandroidmap valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.testandroidmap.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
